...
const stageTooltip: Record<string, string> = {
  Signal: "Detected by W9 competitor radar (Peec delta or Tavily search)",
  Draft: "Counter-narrative generated by LLM with brand voice",
  Approve: "Manual review · approve triggers W7 expand event",
...
export function DraftCard({
    ...
    const t =
      status === "approved"
        ? toast.loading("Approve → expanding draft", {
            description: "W7 builds 3 channel variants (blog/X/LinkedIn)",
          })
  ...
  function onUseVariant(variantId: string, rank: number) {
    ...
    const t = toast.loading(`Using variant #${rank} → expanding`, {
      description: "Draft body is replaced with the variant, W7 builds 3 channel versions",
    });
    startTransition(async () => {
      try {
  ...
  function onSimulate() {
    ...
    const t = toast.loading("Simulating alternatives", {
      description: "W5 ranking 3 variants by mention rate × position × sentiment",
    });
    startTransition(async () => {
      try {
      ...
      {variants.length > 0 ? (
        <div className="mt-3 rounded-md border border-border bg-muted/30 p-2 text-xs">
          <p className="mb-2 font-semibold text-muted-foreground">
            📤 {variants.length} channel variants (W7 expand · blog · X · LinkedIn) — click to expand

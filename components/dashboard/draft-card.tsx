...
const stageTooltip: Record<string, string> = {
  Signal: "Detected by the competitor radar (Peec delta or Tavily search)",
  Draft: "Counter-narrative generated by LLM with brand voice",
  Approve: "Manual review · approve triggers multi-channel expand",
  Expand: "Multi-channel expand generates 3 channel variants: blog / X / LinkedIn",
  Publish: "Mark variant as published in content_variants.status",
};
...
export function DraftCard({
  ...
  // Track in-flight async operations to show inline progress.
  // Baselines store the variant count at click time — when real-time
  // refresh brings new rows, count > baseline → progress complete.
  ...
  useEffect(() => {
    if (expanding && variants.length > expandBaseline.current) {
      setExpanding(false);
      // Auto-expand once variants arrive so the user sees them immediately.
      setExpanded(true);
    }
    ...
    const t =
      status === "approved"
        ? toast.loading("Approve → expanding draft", {
            description: "Multi-channel expand builds 3 variants (blog/X/LinkedIn)",
          })
      ...
      try {
        ...
        if (result.ok) {
          toast.success(status === "approved" ? "Draft approved → expansion triggered" : "Draft rejected", {
            id: t,
            description: status === "approved" ? "Variants will appear in ~90s" : undefined,
          });
        } else {
  ...
  function onUseVariant(variantId: string, rank: number) {
    ...
    const t = toast.loading(`Using variant #${rank} → expanding`, {
      description: "Draft body is replaced with the variant, then 3 channel versions are built",
    });
    startTransition(async () => {
      try {
        ...
        if (result.ok) {
          toast.success(`Variant #${rank} approved → expansion triggered`, {
            id: t,
            description: "Channel variants will appear in ~90s",
          });
        } else {
      ...
      try {
        ...
        if (result.ok) {
          toast.success("Published. Stepper closed.", {
            id: t,
            description: "All channel variants marked as sent",
          });
        } else {
  ...
  function onSimulate() {
    ...
    const t = toast.loading("Simulating alternatives", {
      description: "Ranking 3 variants by mention rate × position × sentiment",
    });
    startTransition(async () => {
      try {
        ...
        if (result.ok) {
          toast.success("Simulator triggered", {
            id: t,
            description: "Variants will appear in Simulator outputs in ~60s",
          });
        } else {
        ...
        <div id={`draft-body-${draft.id}`}>
          ...
          {signal ? (
            <a
              ...
              title="View signal in Signals tab"
            >
      ...
      {narrativeVariants.length > 0 ? (
        <details className="mt-3 rounded-md border border-border bg-muted/30 p-2 text-xs" open>
          <summary className="cursor-pointer font-semibold text-muted-foreground hover:text-foreground">
            ↻ {narrativeVariants.length} simulator variants — pick one to launch expansion
          </summary>
      ...
      {variants.length > 0 && optimisticStatus === "approved" && variants.every((v) => v.status !== "sent") ? (
        <div className="mt-3 flex flex-col gap-1 rounded-md border border-emerald-200 bg-emerald-50/50 p-2 text-xs dark:border-emerald-900 dark:bg-emerald-950/20 sm:flex-row sm:items-center sm:justify-between">
          <span className="text-muted-foreground">
            All 3 channel variants ready. Close the loop?
          </span>
          <Button
            size="sm"
            onClick={onPublish}
      ...
      {variants.length > 0 ? (
        <div className="mt-3 rounded-md border border-border bg-muted/30 p-2 text-xs">
          <p className="mb-2 font-semibold text-muted-foreground">
            📤 {variants.length} channel variants (blog · X · LinkedIn) — click to expand
                    ...
                    <div className="border-t border-border p-2">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          toast.success(`${channelLabel(v.channel)} copied`);
                        }}
